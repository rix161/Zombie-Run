...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		while (!toExplore.isEmpty()) {
			...
			if(!visited.contains(next)){
				visited.add(next);
				Set<MapNode> neighbours = next.getNeighbors();
				
				for(MapNode neig:neighbours){
					double eValue = next.getEdgeLength(neig);
					int val = Double.compare(eValue+next.getDistance(), neig.getDistance());
					if( val < 0){
						neig.setDistance(eValue+next.getDistance());
						toExplore.add(neig);
						parentMap.put(neig, next);
					}
				}
			}
		
		}
		System.out.println("Dijk count:"+count);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		// TODO: Implement this method in WEEK 3
				...
				PriorityQueue<MapNode> toExplore = new PriorityQueue<MapNode>();
				HashSet<MapNode> visited = new HashSet<MapNode>();
				startNode.setActualDistance(endNode.computeActualDistance(startNode));
				startNode.setDistance(0);
				toExplore.add(startNode);
				...
				while (!toExplore.isEmpty()) {
					...
					if(!visited.contains(next)){
						...
						for(MapNode neig:neighbours){
							double eValue = next.getEdgeLength(neig);
							neig.setActualDistance(endNode.computeActualDistance(neig));
							int val = Double.compare(eValue+next.getDistance()+neig.getActualDistance(), neig.getDistance());
							if( val < 0 ){
								neig.setDistance(eValue+neig.getActualDistance()+next.getDistance());
								toExplore.add(neig);
								parentMap.put(neig, next);
							}
						}

...
class MapNode implements Comparable
{
	...
	MapNode(GeographicPoint loc)
	{
		...
		actualDistance = Double.MAX_VALUE;
	}
	...
	public double getEdgeLength(MapNode neigh) {
		for(MapEdge lEdge:this.edges){
			if(lEdge.getEndNode().equals(neigh))
				return lEdge.getLength();
		}
		return Double.MAX_VALUE;
	}
	
	public double computeActualDistance(MapNode endNode){
		GeographicPoint src = this.location;
		GeographicPoint dest = endNode.getLocation();
